...
\\ See css-old.gp (ask me for a copy) for a version which was simpler, more elegant, and
...
is_disc_good(D, X_13) = 
  ...
  if (ifd, 
    if (tf == 1,
      retval = if( absD <= (1/3)*X_13[length(X_13)], -3*D, 0));
    if (tf == 3,
...
\\ compute M_1(s) as in (2.2), for discriminant D and to a length of N.
{
M_one(D, N) = 
  local(v);
  v = vector(N);
  v[1] = 1;
  if (gcd(D, 3) == 3 && N > 2, v[3] = 2);
  if (Mod(D, 9) == Mod(6, 9) && N > 8, v[9] = 6);
  if (gcd(D, 3) == 1 && N > 8, v[9] = 2);
  v;
}    

\\ compute M_{2, E}(s) as in (2.2), for discriminant D and to a length of N.
{
M_two_E(D, N, E) = 
  local(g, n, v);
  v = vector(N);
  ...
  n = gcd(27, E.disc);
  if (N > 8, 
   if (gcd(g, 3) == 1 && n < 27, v[9] = 2);
   if (g == 3 && n < 27, v[3] = 2);
   if (g == 6 && n < 27, v[3] = 2; v[9] = 3 * omega_E(E, 3));
   if (gcd(g, 3) == 1 && n == 27, v[9] = -1);
   if (g == 3 && n == 27, v[3] = -1);
   if (g == 6 && n == 27, v[3] = -1);
  );
  if (N < 9 && N > 2,
...
vector_add(v, N) = 
  local(i, retval);    
  retval = vector(length(N));
  placeholder = 1;
  for (i = 1, length(v), 
    while (i > N[placeholder], placeholder = placeholder + 1);
    for (j = placeholder, length(N), retval[j] = retval[j] + v[i]);
  );
  retval;
}
  ...
  for (i = 1, num_input_files, 
    ...
    for (j = 1, length(v), 
      ...
      if (related_fd != 0, \\ note: manipulate the polynomial slightly to make monic and avoid a warning.
        ...
        partial_sum = vector_add(dir_ser, N); \\ no a priori reason this should be positive, by the way.
	running_total = running_total + partial_sum;
    ));
  );

  for (D = 3, X_13t[length(X_13t)],
    if (isfundamental(compute_sign * D), 
      N = truncate(X_14 / D^(3/4)); 
      dir_ser = compute_main_dirichlet(compute_sign * D, N[length(X)]);
      partial_sum = vector_add(dir_ser, N);
      running_total = running_total + partial_sum;
      for (j = 1, length(X), if (N[j] > 0, running_total[j] = running_total[j] - 1/2));
      \\ print("D = ", D, "; N = ", N, " total = ", partial_sum - 1/(2*c_D(compute_sign * D)));
  ));
  for (i = 1, length(X), 
    print("Number of fields with |Disc(K)| < ", X[i], "; sign = ", \
      compute_sign, ": ", running_total[i]);
  );
}
...
Xtemp = [1, 2, 5];
Xtest = vector(19);
for (ii = 1, 6, for (jj = 1, 3, Xtest[3*(ii - 1) + jj] = 10^(16 + ii)*Xtemp[jj]));
Xtest[19] = 10^23;
